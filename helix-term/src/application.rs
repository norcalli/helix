...
impl SocketResponse {
    async fn respond(self, responder: &Sender<Vec<u8>>) -> () {
        let bytes = match self {
            Self::Bytes(bytes) => bytes,
            Self::Json(json) => json.to_string().into_bytes(),
        };
        let _ = responder.send(bytes).await;
    }
}
...
pub async fn execute_socket_commands(
    ...
    for command in commands.into_iter() {
        match command.name() {
            "socket-read-buffer" => {
                socket_read_buffer(ctx.editor).respond(&responder).await;
            }
...
struct CommandListener {
    socket_path: PathBuf,
    rx: Receiver<(Vec<MappableCommand>, Sender<Vec<u8>>)>,
}
...
fn spawn_command_listener(
    socket_path: PathBuf,
) -> Receiver<(Vec<MappableCommand>, Sender<Vec<u8>>)> {
                                ...
                                loop {
                                    ...
                                    match result {
                                        ...
                                        Err(err) => {
                                            ...
                                            let mut sliced = errs.as_str();
                                            if sliced.len() > u16::MAX as usize {
                                                for i in 0..sliced.len() {
                                                    let Some((head, _tail)) =
                                                        sliced.split_at_checked(sliced.len() - i)
                                                    else {
                                                        continue;
                                                    };
                                                    sliced = head;
                                    ...
                                    while let Some(response) = response_rx.recv().await {
                                        ...
                                        write.write(b"b").await?;
                                        write.write_u32_le(response.len() as u32).await?;
